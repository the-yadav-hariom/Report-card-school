...
const ReportCard = ({ student, school }) => {
  ...
  return (
    <div className="report-card-printable bg-paper p-4 sm:p-8 rounded-xl border border-paper-border shadow-lg font-body max-w-5xl mx-auto my-2 text-gray-900 print:p-3 print:my-0">
      
      {/* 1. INSTITUTIONAL HEADER */}
      <div className="border-b-2 border-maroon pb-2.5 mb-3 print:pb-2 print:mb-3 flex flex-col sm:flex-row items-center justify-between gap-2 text-center sm:text-left">
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <img 
            src={school?.schoolLogo || "/mahaviri_shishu_vidya_mandir_logo/screen.png"} 
            alt="School Crest" 
            className="w-14 h-14 print:w-16 print:h-16 object-contain rounded bg-white p-0.5 border border-maroon/20 shadow-xs"
          />
          <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 mb-3.5 print:mb-3 border border-gray-300 rounded-lg p-2.5 print:p-2 bg-white/80 shadow-2xs">
        {/* Photo / Initials */}
        <div className="flex justify-center sm:justify-start items-center">
          {student.studentPhoto ? (
            <div className="w-36 h-36 print:w-28 print:h-28 rounded-lg overflow-hidden border-2 border-maroon/40 shadow-sm bg-white p-0.5 shrink-0">
              <img src={student.studentPhoto} alt={student.studentName} className="w-full h-full object-cover rounded" />
            </div>
          ) : (
            <div className="w-36 h-36 print:w-28 print:h-28 rounded-lg bg-maroon/10 border-2 border-maroon/30 text-maroon font-extrabold text-2xl print:text-xl flex items-center justify-center shadow-inner shrink-0">
              {student.initials || 'ST'}
            </div>
          )}
        </div>
      ...
      <div className="mb-3.5 print:mb-3 overflow-x-auto">
        <h3 className="font-heading font-bold text-xs print:text-[11px] text-maroon mb-1 print:mb-1 uppercase tracking-wide border-b border-maroon/30 pb-0.5">
            ...
            {student.scholastic && student.scholastic.map((sub, idx) => {
              ...
              return (
                <tr key={idx} className="hover:bg-maroon/5 transition-colors">
                  ...
                  {/* Term 1 */}
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.per1}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.nb1}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.sea1}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5 font-semibold bg-gray-50">{testTot1}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.hy1}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5 font-bold text-maroon bg-maroon/5">{tot1}</td>

                  {/* Term 2 */}
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.per2 ?? '-'}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.nb2 ?? '-'}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.sea2 ?? '-'}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5 font-semibold bg-gray-50">{sub.yr2 ? testTot2 : '-'}</td>
                  <td className="border border-gray-300 p-1 print:p-1.5">{sub.yr2 ?? '-'}</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3 print:mb-3 print:gap-3">
            ...
            <tbody className="divide-y divide-gray-200 font-bold text-gray-800">
              {student.coScholastic && student.coScholastic.map((item, idx) => (
                <tr key={idx}>
                  ...
                  <td className="p-1.5 print:p-1 text-center border-r border-gray-300 text-emerald-700">{item.hyGrade}</td>
                  <td className="p-1.5 print:p-1 text-center text-emerald-700">{item.annualGrade}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="w-full text-[10px] print:text-[9.5px] text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-200 text-[10px] print:text-[9.5px] text-gray-800 font-medium">
              <tr>
                ...
                <td className="p-1.5 print:p-1 text-center border-r border-gray-300">{student.attendance?.t1Present || 0} / {student.attendance?.t1Total || 60}</td>
              ...
              <tr>
                ...
                <td className="p-1.5 print:p-1 text-center border-r border-gray-300">{student.attendance?.hyPresent || 0} / {student.attendance?.hyTotal || 60}</td>
                <td className="p-1.5 print:p-1 text-center font-bold text-maroon">
              ...
              <tr>
                ...
                <td className="p-1.5 print:p-1 text-center border-r border-gray-300">{student.attendance?.t2Present || 0} / {student.attendance?.t2Total || 60}</td>
                <td className="p-1.5 print:p-1 text-center font-bold text-maroon">
              ...
              <tr>
                ...
                <td className="p-1.5 print:p-1 text-center border-r border-gray-300">{student.attendance?.annualPresent || 0} / {student.attendance?.annualTotal || 60}</td>
                <td className="p-1.5 print:p-1 text-center font-bold text-maroon">
      ...
      <div className="border-t border-gray-300 pt-2.5 mt-2.5 print:pt-2 print:mt-2">
        <div className="mb-2 print:mb-2 p-2 print:p-1.5 bg-white border border-gray-300 rounded-lg text-[10px] print:text-[9.5px]">
        ...
        <div className="grid grid-cols-3 gap-2 text-center text-[10px] print:text-[9px] text-gray-700 font-bold pt-3 print:pt-2">
          <div>
